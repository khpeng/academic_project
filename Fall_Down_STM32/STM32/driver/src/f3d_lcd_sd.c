 ...
 * Maintainer: Yifan Liu: liuifa ; Shengwei Peng: pengs
...
void f3d_lcd_sd_interface_init(void) {
 /* vvvvvvvvvvv pin initialization for the LCD goes here vvvvvvvvvv*/ 
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_12 | GPIO_Pin_8; 
...
void f3d_lcd_bar(uint8_t x, uint8_t y, uint8_t xMax, uint8_t yMax, uint16_t color) {
  ...
  /*uint16_t bar[yMax-y];
  for (y;y<xMax;y++) bar[y] = color;
  f3d_lcd_setAddrWindow(x,y,xMax,yMax,MADCTLGRAPHICS);
  for (y;y<yMax; y++) {
      f3d_lcd_pushColor(bar,yMax-y);
  }*/
}
...
//////////////////////////////////////
void f3d_lcd_drawChar_DOT(uint8_t x, uint8_t y, unsigned char c, uint16_t color, uint16_t background_color) {
  int i, j;
...
void f3d_lcd_drawString_DOT(uint8_t x, uint8_t y, char *c, uint16_t color, uint16_t background_color) {
  while (c && *c) {
    f3d_lcd_drawChar_DOT(x, y, *c++, color, background_color);
    x += 6;
    if (x + 5 >= ST7735_width) {
      y += 10;
      x = 0;
